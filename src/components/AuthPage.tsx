...
export default function AuthPage({ initialView, usernameClaimed = '', onNavigate, onSuccess }: AuthPageProps) {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    // Pre-store pending signup profile with 7-day reverse trial
    ...
    try {
      sessionStorage.setItem('ryyco_pending_signup_profile', JSON.stringify(pendingData));
    } catch (e) {}
    ...
    try {
      ...
      localStorage.setItem('ryyco_auth_mode', 'seller');
      localStorage.setItem(`linnk_session_${uid}`, JSON.stringify(newProfile));
      try {
        const localProfiles = JSON.parse(localStorage.getItem('linnk_profiles') || '{}');
        localProfiles[newProfile.username] = newProfile;
