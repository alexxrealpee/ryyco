            ...
            >
              ...
              <div className="bg-gray-950/60 border border-gray-850 p-3 rounded-xl space-y-1.5 text-xs text-gray-300">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {dr.phone && (
                    <a
                      href={`https://wa.me/${dr.phone.replace(/\D/g, '').startsWith('57') ? dr.phone.replace(/\D/g, '') : '57' + dr.phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Hola ${dr.firstName}! Te contactamos desde la administración general de RYYCO.`)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-2 py-0.5 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 font-bold text-[10px] flex items-center gap-1 transition"
